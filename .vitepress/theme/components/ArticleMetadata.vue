 ...
 * ArticleMetadata 组件
 *
 * 自动在每篇文章的 <h1> 标题下方展示文章元信息：
 * - 最后更新时间（基于 VitePress 的 lastUpdated）
 * - 文章字数（中英文分别统计）
 * - 预计阅读时长
 *
 * 该组件由 .vitepress/config.mjs 中的 markdown.heading_close 规则自动注入。
 */
import { useData } from 'vitepress'
import { computed, ref, onMounted } from 'vue'
import { countWord } from '../untils/functions.js'

// 获取当前页面数据，包含 lastUpdated 等元信息
const { page } = useData()
...
// 按每分钟 275 字的阅读速度估算文字阅读时间（秒）
const wordTime = computed(() => {
  return ((wordCount.value / 275) * 60)
})

// 图片阅读时间：前 10 张依次递增，超过 10 张后每张按 3 秒计算
...
<template>
  <div class="word">
    <p style="display: flex;gap: 13px">
      <!-- 当 lastUpdated 有效时才显示更新时间 -->
      <span  v-if="date.toDateString()!= 'Invalid Date'">
