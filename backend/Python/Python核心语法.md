...
传参方式指的是，再调用函数时，传递实参的方式
1. 位置参数：调用函数时根据函数定义时的位置来传递参数
   * 优点：简洁
   * 缺点：可读性差、易出错、维护难
   * 场景：参数少（不超过3个），且顺序自然
2. 关键字传参：调用函数时以函数定义时形参名称作为关键字，以 “ 键 = 值 ” 的形式来传递参数（不要求顺序）
   * 优点：可读性强、易维护和扩展
   * 缺点：代码繁琐
   * 场景：参数较多、或易混淆的场景
...
默认参数也称为缺省参数，用于在定义函数时，为参数提供默认值，调用函数时，可以不传递默认值的参数
...
* 匿名函数中可以返回结果，也可以不返回结果。返回结果是，不需要写return，表达式的运行结果就是要返回的结果
...
类属性：类属性时属于类本身的属性，所有实例共享的。（所有对象共享的数据或配置）
...
**私有**：私有的属性和方法只能在类的内部使用；Python中并没有真正的私有机制，约定在私有属性名和方法名前加`__`（两个下划线）
...
**多继承**：指的时一个子类，同时继承了多个父类的情况（会将多个父类中的非私有的属性和方法都继承下来）
...
注意：当一个类继承了多个父类时，默认优先使用第一个父类中的同名属性或方法，可以使用`类名.__属性`或`类名.__mro__`方法来查看调用顺序
...
异常（也称Bug）就是程序运行过程中出现的错误，它会终端程序的正常执行流程
作用：
...
异常的传递就是异常在函数调用中层层上报的过程，直到有人处理它，或者程序崩溃
